...
public :
  ...
  VSVR2 ( const int tex_ni = -1, const int tex_nj = -1, const int tex_nk = -1, const int tf_size = -1 ) :
...
void main() {
  ...
  float value = float(texture3D(voltex, gl_TexCoord[0].stp));
  gl_FragColor = texture1D(transf, value);
   //= c;//vec4(v.r, v.r, v.r, 0.5);//c; 
}
)";
		printf("gothere 4\n");
		//textFileRead("volshader.frag");
		 //printf(" fs = %s\n\n", fs);
		const char* fv = fs;
		/*= "uniform sampler3D voltex; \
						 uniform sampler1D transf; \
						 ...
						 vec4 c = texture1D(transf, value / 256.0 ); \
						 gl_FragColor = c ; }";*/

		glShaderSourceARB(f, 1, &fv, NULL);
		glCompileShaderARB(f);

		// check if this compiled
		GLint compiled;
		glGetObjectParameterivARB(f, GL_COMPILE_STATUS, &compiled);
		if (!compiled) {
			...
			if (blen > 1) {
				...
				glGetInfoLogARB(f, blen, &slen, compiler_log);
				printf(" ---- compiler_log: \n %s \n", compiler_log);
				free(compiler_log);
			}
		}
		...
		glAttachObjectARB(_sprog, f);
		glLinkProgramARB(_sprog);

		printf(" - VSVR initialized\n");
		// will attach to texture units 1 and 2
  }

  void SetMinMax(float minimum, float maximum) {
	  this->_tf_max = maximum;
	  this->_tf_min = minimum;
  }
  /** Destructor */
  ~VSVR2() { tex_glunload() ;  tf_glunload() ;  tex_free() ;  tf_free() ; }
...
public :
   ...
   * \param nslices number of slices cut into the texture
   */
  bool gl_redisplay ( int nslices = 256 ) ;
...
public :
  ...
  void tex_change(float* data) {
	  for (size_t i = 0; i < _tex_ni * _tex_nj * _tex_nk; i++) _tex[i] = data[i];
	  _tex_changed = true;
  }
