...
| `_stage_field(key)` / `_stage_measure(key)` / `_measure_key()` | `("none", "")` / `("none", "")` / None | the **stage strip** (`panels/stages.py`): the app judges the msc and stats boxes for an item as `(state, tip[, text])` and names its measurement (coupon / mspath: the measurement fingerprint of the panel); the framework judges the model box (compat gate, `ModelStack.trained_rev` / `trained_measure` stamped by `_install_model`) and the classified box (`pred[key][0] == rec["commit"]`), then `propagate` turns what is downstream of a stale / busy / failed box stale. `STAGE_STRIP` (AnnotationShell) tells the apps' `_update_busy` to leave stage states to the strip. `SliceCanvas.set_stages` draws it; a click opens the box's tab (`on_stage_click` -> `_show_center_tab`); `_compute_badge(text, stage=)` spins a box instead of writing the HUD |
