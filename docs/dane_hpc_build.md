...
- **MSCEER / GInt errors under GCC** (`constexpr operator[]` redeclaration in
  ...
  MSCEER is C++11-era code that only ever built on MSVC. This is fixed upstream
  as of the pinned MSCEER revision (`a77e864`+), which adds the missing
  `#include <cstring>` to the affected headers and requires C++17 via
  `target_compile_features(... PUBLIC cxx_std_17)` on the `GInt` / `msc_2d_lib`
  targets (portable to MSVC). No MSSeg-side workaround is needed. If you pin an
  *older* MSCEER revision and hit these, either bump the pin or re-add the
  target-level override in `cmake/Dependencies.cmake` after the MSCEER
  `add_subdirectory` (bump `CXX_STANDARD` to 17 and force-include `<cstring>`,
  guarded by `if(NOT MSVC)`).
