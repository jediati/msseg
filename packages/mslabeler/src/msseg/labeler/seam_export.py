...
def class_name(k) -> str:
    k = int(k)
    return SEAM_CLASSES[k] if 0 <= k < len(SEAM_CLASSES) else str(k)


def seam_rows(graph, seam_class, boundaryness, np):
    """One dict per seam: ``id, a, b, j0, j1, length, class, class_name,
    boundaryness, points`` (points in image coordinates)."""
...
def item_doc(key, graph, seam_class, boundaryness, np):
    pl = graph.placement
    return {"item": str(key), "shape": [int(graph.height), int(graph.width)],
            "placement": {"origin": [pl.ox, pl.oy], "scale": pl.scale},
            "classes": list(SEAM_CLASSES),
            "n_seams": int(graph.n_seams), "n_junctions": int(graph.n_junctions),
            "seams": seam_rows(graph, seam_class, boundaryness, np)}


def write_seams_json(path, key, graph, seam_class, boundaryness, np):
    doc = item_doc(key, graph, seam_class, boundaryness, np)
